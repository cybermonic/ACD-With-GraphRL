...
from models.ppo import GraphPPOAgent
from models.memory_buffer import PPOMemory
    

class NaiveInductiveActorNetwork(nn.Module):
    def __init__(self, in_dim, gnn_type, num_nodes=13, action_space=11, n_global_actions=2,
        ...
        if gnn_type == 'gcn': 
            ...
            self.conv1 = GATConv(in_dim, hidden1)
            ...
            raise ValueError("gnn_type must be one of ['gcn', 'sage', 'gat', 'gin']")
...
class InductiveCriticNetwork(nn.Module):
    def __init__(self, in_dim, gnn_type, num_nodes=13, 
...
class GlobalNodeInductiveActorNetwork(NaiveInductiveActorNetwork):
    def __init__(self, in_dim, gnn_type, num_nodes=13, action_space=11, n_global_actions=2,
                 hidden1=256, hidden2=64, lr=0.0003):
        super().__init__(in_dim, gnn_type, num_nodes=num_nodes, action_space=action_space, hidden1=hidden1, hidden2=hidden2, lr=lr)
        self.N_GLOBAL_ACTIONS = n_global_actions
...
class GlobalNodeInductiveCriticNetwork(InductiveCriticNetwork):
    def __init__(self, in_dim, gnn_type, num_nodes=13, 
                 hidden1=256, hidden2=64, lr=0.001):
        super().__init__(in_dim, gnn_type, num_nodes, hidden1, hidden2, lr)
        gdim = hidden1
...
class InductiveGraphPPOAgent(GraphPPOAgent):
    def __init__(self, in_dim, gnn_type, gamma=0.99, lmbda=0.95, clip=0.1, bs=5, epochs=6, 
                 a_kwargs=dict(), c_kwargs=dict(), training=True, naive=True, globalnode=False):
        if naive and not globalnode:
            self.actor = NaiveInductiveActorNetwork(in_dim, gnn_type, **a_kwargs)
            self.critic = InductiveCriticNetwork(in_dim, gnn_type, **c_kwargs)
        elif globalnode: 
            self.actor = GlobalNodeInductiveActorNetwork(in_dim, gnn_type, **a_kwargs)
            self.critic = GlobalNodeInductiveCriticNetwork(in_dim, gnn_type, **c_kwargs)
        else:
            raise NotImplementedError("We should never arrive at this part of the code")

        self.memory = PPOMemory(bs)

        self.args = (in_dim, gnn_type)
        self.kwargs = dict(
