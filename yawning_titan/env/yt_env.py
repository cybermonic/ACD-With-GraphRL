...
def build_graph(num_nodes, p=0.1, min_v=0.01, max_v=1, recurse=0, verbose=True, add_degree=False):
    ...
    # Add blank label so GCN can count neighbors easier
...
class YTEnv:
    ...
    def step(self, blue_action, blue_target):
        ...
        return r+big_r, r, terminate, self.state()
    ...
    def attack(self, nids):
        for nid in nids:
            # When strength == 0.5 this is between 0.21 and 0.5
            attack_strength = (
                self.RED_SKILL**2 /
                (self.RED_SKILL + (1-self.x[nid, self.VULN]) )
            )

            # Success
            t = random.random()
            if attack_strength >= t:
...
class BlueAgent:
    def __init__(self, env, model, deterministic=False, inductive=False, only_restore=False, value_network=False):
        self.env = env
        self.model = model
        self.deterministic = deterministic
        self.inductive = inductive
        self.only_restore = only_restore
        self.value_network = False # Otherwise assume policy net

    def num_to_action(self, i):
        if self.only_restore:
            return self.num_to_restore(i)
        if self.inductive:
    ...
    def num_to_restore(self, i):
        return self.env.restore, i

    def select_dqn_action(self, x,ei):
        actions = self.model.get_action(x,ei, x.size(0))
        return self.num_to_action(actions), actions
