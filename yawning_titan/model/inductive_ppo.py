...
class ActorNetwork(nn.Module):
    def __init__(self, in_dim, action_space,
        ...
        self.opt = SGD(self.parameters(), lr)
...
class InductiveGraphPPO(GraphPPO):
    def __init__(self, in_dim, action_space, batch_size,
                 gamma=0.99, lmbda=0.95, clip=0.2, alr=0.005, clr=0.01, attn=False):
        self.args = (in_dim, action_space, batch_size)
        self.kwargs = dict(gamma=gamma, lmbda=lmbda, clip=clip, alr=alr, clr=clr, attn=attn)

        if not attn:
            self.actor = ActorNetwork(in_dim, action_space, lr=alr)
            self.critic = CriticNetwork(in_dim, lr=clr)
        else:
            self.actor = GlobalNodeActorNetwork(in_dim, action_space, lr=alr)
            self.critic = GlobalNodeCriticNetwork(in_dim, lr=clr)
    ...
    def forward(self, x,ei, num_nodes):
        probs = self.actor(x,ei,num_nodes)
        val = self.critic(x,ei, num_nodes)
        return probs,val
...
class SimpleSelfAttention(nn.Module):
    ...
    def forward(self, v, g=None):
        ...
        out = (att*feat).mean(dim=1)    # B x h

        g_ = self.glb(torch.cat([out,g], dim=-1))  # B x g
        return g + g_                               # Short-circuit


class GlobalNodeActorNetwork(ActorNetwork):
    def __init__(self, in_dim, action_space,
                 hidden1=32, hidden2=32, lr=0.005):
        super().__init__(in_dim, action_space, hidden1, hidden2, lr)
    ...
    def __reshape_hosts(self, x, num_nodes):
        nbatches = x.size(0) // num_nodes
        return x.reshape(nbatches, num_nodes, x.size(1))

    def forward(self, x, ei, num_nodes):
        g = self.g0_attn(self.__reshape_hosts(x, num_nodes))
        x = torch.relu(self.conv1(x, ei))

        g = self.g1_attn(self.__reshape_hosts(x, num_nodes), g=g) # B x h2
        x = torch.relu(self.conv2(x, ei))

        g = self.g2_attn(self.__reshape_hosts(x, num_nodes), g=g) # B x h2
        g = g.repeat_interleave(num_nodes, 0)      # B*N x h2

        host_z = torch.cat([x, g], dim=1)        # B*N x h2*2
        actions = self.out(host_z)               # B*N x a

        nbatches = actions.size(0) // num_nodes
        dist = actions.reshape(nbatches, num_nodes*actions.size(1))
        dist = self.softmax(dist)
...
class GlobalNodeCriticNetwork(CriticNetwork):
    def __init__(self, in_dim, hidden1=32, hidden2=32, lr=0.001):
        super().__init__(in_dim, hidden1, hidden2, lr)
        gdim = hidden1
    ...
    def forward(self, x, ei, num_nodes):
        ...
        h = self.__reshape_hosts(x, num_nodes)  # B x N x h1
        g = self.g1_attn(h, g=g)            # B x h2
        x = torch.relu(self.conv2(x, ei))   # B*N x h2

        h = self.__reshape_hosts(x, num_nodes)  # B x N x h2
        g = self.g2_attn(h, g=g)            # B x h2
        return self.out(g).squeeze(-1) # B x 1
