...
from model.ppo import GraphPPO, PPOMemory
...
def experiment(env: YTEnv, agent: BlueAgent, domain_randomization=False):
    ...
    while tr_steps < MAX_STEPS:
        ...
        torch.save({'rews': ep_rews, 'lens': ep_lens}, f'logs/transductive_{GRAPH_SIZE}N_{SEED}.pt')
        ...
        agent.model.save(f'saved_models/ppo_transductive_{GRAPH_SIZE}N_{SEED}_last.pt')
...
if __name__ == '__main__':
    ...
    blue = GraphPPO(x.size(0),x.size(1), 2*GRAPH_SIZE, BATCH_SIZE, alr=A_LR, clr=C_LR, clip=CLIP)
